...
import ru.yourteam.filmorate.mapper.UserRowMapper;
import ru.yourteam.filmorate.mapper.review.FilmRowMapper;
import ru.yourteam.filmorate.mapper.review.ReviewRowMapper;
...
public class ReviewRepository {
    ...
    private static final String GET_ALL_SORTED_REVIEWS =
        "SELECT r.review_id, r.content, r.is_positive, r.user_id, r.film_id, "
            + "COALESCE(SUM(CASE WHEN rr.is_positive = true THEN 1 ELSE -1 END), 0) as useful "
            + "FROM reviews r "
            + "LEFT JOIN review_ratings rr ON r.review_id = rr.review_id "
            + "GROUP BY r.review_id, r.content, r.is_positive, r.user_id, r.film_id "
            + "ORDER BY useful DESC LIMIT ?";
    private static final String ADD_LIKE_TO_REVIEW_QUERY =
        "INSERT INTO review_ratings (review_id, user_id, is_positive) VALUES (?,?,true)";
    private static final String ADD_DISLIKE_TO_REVIEW_QUERY =
        "INSERT INTO review_ratings (review_id, user_id, is_positive) VALUES (?,?,false)";

    private static final String DELETE_LIKE_FROM_REVIEW_QUERY =
    ...
    public List<Review> getReviewsByFilmId(Integer filmId, int count) {
        if (filmId == null) {
            return jdbc.query(GET_ALL_SORTED_REVIEWS, rowMapper, count);
        } else {
